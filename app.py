...
Thin wrapper over fatigue_calc.py - all math lives there.
"""
...
from fatigue_calc import Task, get_model, MODELS
...
FIELD_DEFINITIONS = {
    "lifft": [
        {"name": "load_kg", "label": "Load (kg)", "type": "number", "min": 0, "step": 0.1},
        {"name": "distance_m", "label": "Horizontal Distance from Spine (m)", "type": "number", "min": 0, "step": 0.01},
        {"name": "reps", "label": "Repetitions per Workday", "type": "number", "min": 0, "step": 1},
    ],
    "duet": [
        {"name": "omni", "label": "OMNI-RES Effort Rating", "type": "select", "options": [
    ...
    "shoulder": [
        {"name": "load_lb", "label": "Load / Force (lb)", "type": "number", "min": 0, "step": 0.1},
        {"name": "distance_in", "label": "Distance from Acromion (in)", "type": "number", "min": 0, "step": 0.1},
        {"name": "reps", "label": "Repetitions per Workday", "type": "number", "min": 0, "step": 1},
...
def assess(req: AssessRequest):
    try:
        ...
        raise HTTPException(400, f"Unknown model: {req.model}. Available: {', '.join(MODELS)}")
