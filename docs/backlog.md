...
- `fl report` - generate a PDF or HTML summary with risk gauges, suitable for handing to a plant manager
- Migrate argparse to typer or click for better CLI ergonomics
- Use rich for formatted terminal output (tables, color-coded risk levels)
- Add `py.typed` marker and strict type annotations for downstream package users
- Pre-commit hooks (ruff, pytest) instead of manual lint/test commands

## Medium effort, builds on what's here

- Rebuild web UI - the current single-file vanilla JS frontend was fine for a 30-minute demo but needs a real framework. Two good options that preserve the zero-JS-build-step, single-`fl serve` story:
  - *htmx + Jinja templates + Pico/Simple.css* - server-rendered, minimal JS, clean look
  - *FastHTML* - pure Python, no HTML/JS to write, integrates with the existing FastAPI backend
...
- Containerize with a Dockerfile for easy deployment beyond localhost
